...
const carsData = [
  { id:'c1', make:'Toyota', model:'Corolla Axio', year:2018, price:8500, mileage:'58,000 km', transmission:'Automatic', engine:'1.5L', color:'Silver', location:'Tokyo', images:['assets/images/car1.jpg','assets/images/car2.jpg'], desc:'Well-maintained Toyota Corolla Axio. Economical and reliable.'},
  { id:'c2', make:'Nissan', model:'Note e-Power', year:2019, price:11200, mileage:'42,000 km', transmission:'CVT', engine:'1.2L hybrid', color:'White', location:'Osaka', images:['assets/images/car2.jpg','assets/images/car3.jpg'], desc:'Comfortable hatchback with economical hybrid powertrain.'},
  { id:'c3', make:'Honda', model:'Fit', year:2017, price:7200, mileage:'69,000 km', transmission:'Automatic', engine:'1.3L', color:'Blue', location:'Yokohama', images:['assets/images/car3.jpg','assets/images/car1.jpg'], desc:'Compact and practical. Great city car.'}
];
...
function render(list){
  grid.innerHTML='';
  if(!list.length){ grid.innerHTML='<div style="grid-column:1/-1;text-align:center;color:var(--muted)">No cars found</div>'; return; }
...
function openModalById(id){
  ...
  ['engine','transmission','color','mileage'].forEach(k=>{ if(car[k]){ const d=document.createElement('div'); d.className='chip'; d.textContent = `${k.charAt(0).toUpperCase()+k.slice(1)}: ${car[k]}`; modalSpecs.appendChild(d);} });
  modalDesc.textContent = car.desc || '';
  // carousel
  let cur = 0; carouselImg.src = car.images[cur] || '';
  thumbs.innerHTML=''; car.images.forEach((src, idx)=>{ const t = document.createElement('img'); t.src=src; t.dataset.idx=idx; t.addEventListener('click', ()=>{ carouselImg.src = src; document.querySelectorAll('.thumbs img').forEach(i=>i.classList.remove('active')); t.classList.add('active'); }); if(idx===0) t.classList.add('active'); thumbs.appendChild(t); });
